...
# ---------------------------------------------------------------------------
@app.get("/glass/turns")
async def glass_turns():
    """Snapshot of recent turn-hop events, oldest first."""
    return {"turns": session_mgr.glass_snapshot()}


@app.get("/glass/stream")
async def glass_stream():
    """Live SSE feed of turn-hop events across all sessions."""
    async def event_stream():
        async for event in session_mgr.glass_stream():
            yield f"data: {json.dumps(event)}\n\n"
...
@app.get("/glass/fleet")
async def glass_fleet():
    """One card per registered mind: health, sessions, last-reply age."""
    minds = await broker.get_registered_minds(app.state.broker_db)

    async def probe(mind: dict) -> dict:
        url = (mind.get("gateway_url") or "").rstrip("/")
        healthy = False
        if url:
            try:
                ...
                healthy = False
        rows = await session_mgr._db.execute_fetchall(
            """SELECT COUNT(*) AS n, MAX(last_active) AS last
               FROM sessions WHERE mind_id = ? AND status != 'closed'""",
            (mind["id"],),
        )
        n, last = (rows[0][0], rows[0][1]) if rows else (0, None)
        return {
            "mind_id": mind["id"],
            "name": mind.get("name"),
            "gateway_url": url,
    ...
    cards = await asyncio.gather(*(probe(m) for m in minds))
    return {"minds": list(cards)}


@app.get("/glass/logs/{mind_id}")
async def glass_logs(mind_id: str, lines: int = 100):
    """Proxy a mind server's /logs/tail SSE stream (minds are LAN-only)."""
    mind = await broker.get_mind_by_id(app.state.broker_db, mind_id)
    if not mind or not mind.get("gateway_url"):
        return JSONResponse({"error": f"Mind {mind_id} not found"}, status_code=404)
    url = f"{mind['gateway_url'].rstrip('/')}/logs/tail"

    async def event_stream():
        try:
            timeout = aiohttp.ClientTimeout(total=None, sock_read=300)
            async with aiohttp.ClientSession(timeout=timeout) as http:
                async with http.get(url, params={"lines": lines}) as resp:
                    if resp.status != 200:
                        body = (await resp.text())[:200]
                        yield f"data: {json.dumps({'type': 'error', 'detail': f'HTTP {resp.status}: {body}'})}\n\n"
                        return
